...
; Disabled: Preserve backticks instead of converting to $()
; (command_substitution
;   .
...
; NOTE: Disabled $[...] to $(( ... )) rewriting due to zsh grammar alias issue
; The zsh grammar aliases $[ as two separate tokens, causing double replacement
...
; Zsh repeat statement formatting
(repeat_statement
  "repeat" @append_space
  count: (_) @append_space
) @append_hardline

; Ensure the word list is delimited by spaces in select loops
(select_statement
  "in"
  (_) @prepend_space
)
...
; In zsh, we keep the "function" keyword and ensure it's spaced from the name
(function_definition
...
; Note: In zsh grammar, variable_name is inside variable_assignment, not a direct child
; So we handle variable_assignment directly and arguments via the 'argument' field

; In zsh, arguments are wrapped in the 'argument' field
(declaration_command
...
; Concatenations in declarations need spacing
; In bash: variable_name is optional direct child, but in zsh it's inside variable_assignment
; So we just capture concatenation as an argument
(declaration_command
  argument: (concatenation) @prepend_space
)
...
; DISABLED: Convert (variable_ref) in simple forms to use expansion syntax
; This would convert $var to ${var}, which changes user code unnecessarily
; and is not present in bash.scm. Keeping simple forms as-is.
...
;   "$"
;   .
;   (simple_variable_name) @append_delimiter
